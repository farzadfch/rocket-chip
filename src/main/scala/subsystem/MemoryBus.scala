...
case class BankedL2Params(
  ...
  coherenceManager: BaseSubsystem => (TLInwardNode, TLOutwardNode, () => Option[Bool]) = { subsystem =>
    ...
    val BroadcastParams(nTrackers, bufferless, roundRobinA) = p(BroadcastKey)
    val bh = LazyModule(new TLBroadcast(subsystem.memBusBlockBytes, nTrackers, bufferless, roundRobinA))
